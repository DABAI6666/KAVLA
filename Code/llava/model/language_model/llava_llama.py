...
from ..llava_arch import LlavaMetaModel, LlavaMetaForCausalLM, LlavaKnowledgeMetaModel, LlavaKnowledgeMetaForCausalLM, \
    LlavaOPTAttentionKnowledgeMetaForCausalLM, \
    LlavaOPTKnowledgeMetaModel, LlavaOPTKnowledgeMetaForCausalLM, LlavaSCKAOPTAttentionKnowledgeMetaForCausalLM
...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def __init__(self, config):
        ...
        self.scka_interactor = Blip2QFormerModel(qformer_config)
...
class KnowledgeOPTQformerLlavaLlamaForCausalLM(LlamaForCausalLM, LlavaSCKAOPTAttentionKnowledgeMetaForCausalLM):
    config_class = KnowledgeLlavaConfig

    def __init__(self, config):
        super(LlavaSCKAOPTAttentionKnowledgeMetaForCausalLM, self).__init__(config)
        self.model = KnowledgeOPTLlavaLlamaModel(config)
        self.pretraining_tp = config.pretraining_tp
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)

        qformer_config = InstructBlipQFormerConfig(encoder_hidden_size=config.hidden_size,
    ...
    def forward(
            ...
            output_hidden_states: Optional[bool] = None,
            images: Optional[torch.FloatTensor] = None,
            scka_triples: Optional[List[str]] = None,
            return_dict: Optional[bool] = None,
    ) -> Union[Tuple, CausalLMOutputWithPast]:
        ...
        if (input_position_ids == KNOWLEDGE_QUERY_TOKEN_INDEX).sum() != 0:
            ...
            input_position_ids = torch.concat(
                [torch.ones(bsz, image_embedding_len, device=inputs_embeds.device, dtype=input_position_ids.dtype),
        ...
        # qformer
        if scka_triples is not None:
            knowledge_embeds = knowledge_embeds.view(bsz, -1, knowledge_embeds.size(-1))
            print(knowledge_embeds.size())
            knowledge_attention_mask = torch.ones(knowledge_embeds.size()[:-1], dtype=torch.long,
                                                  device=knowledge_embeds.device)
            ...
            scka_attention_mask = torch.ones(query_tokens.size()[:-1], dtype=torch.long, device=query_tokens.device)
            ...
            query_outputs = self.qformer(
                ...
                encoder_attention_mask=scka_attention_mask,
            )
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, inputs_embeds=None, **kwargs):
        ...
        scka_triples = kwargs.pop("scka_triples", None)
        _inputs = super().prepare_inputs_for_generation(
            input_ids, past_key_values=past_key_values, inputs_embeds=inputs_embeds, **kwargs
        )
        if images is not None:
        ...
        if scka_triples is not None:
            _inputs['scka_triples'] = scka_triples
